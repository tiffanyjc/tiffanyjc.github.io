...
__export(src_exports, {
  ...
  getFirstStaticColor: () => getFirstStaticColor,
  getInlineStyles: () => getInlineStyles,
  getLuminance: () => getLuminance,
  getStableObjectHash: () => getStableObjectHash,
  getStaticBackgroundColor: () => getStaticBackgroundColor,
  lighten: () => lighten,
  mix: () => mix,
...
function getInlineStyles(node) {
  ...
  try {
    const root = import_postcss.default.parse(styleString, postCssOptions);
    root.each((node2) => {
      if (node2.type === "decl")
...
function setInlineStyles(node, styles) {
  const styleString = [...styles].map(
    ([prop, value]) => new import_postcss.Declaration({
      prop,
      value,
      raws: {
        between: ":"
...
function renderLineToAst({
  ...
  const renderedGutterElements = sortedGutterElements.map(({ pluginName, gutterElement }) => {
    try {
      ...
      if (!isHastParent(node))
        throw new Error(`renderLine function did not return a valid HAST parent node: ${JSON.stringify(node)}`);
      return node;
    } catch (error) {
      const msg = error instanceof Error ? error.message : error;
  ...
  let lineNode = (0, import_hastscript2.h)(`div.${codeLineClass}`);
  if (renderedGutterElements.length) {
    lineNode.children.push((0, import_hastscript2.h)("div.gutter", renderedGutterElements));
  }
  lineNode.children.push((0, import_hastscript2.h)("div.code", partNodes.length > 0 ? partNodes : (0, import_hastscript2.h)(null, "\n")));
  annotations.forEach((annotation) => {
    if (annotation.inlineRange)
      return;
  ...
  return () => {
    ...
    const renderedGutterElements = sortedGutterElements.map(({ pluginName, gutterElement }) => {
      try {
        ...
        if (!isHastParent(node))
          throw new Error(`renderPlaceholder function did not return a valid HAST parent node: ${JSON.stringify(node)}`);
        return node;
      } catch (error) {
        const msg = error instanceof Error ? error.message : error;
    ...
    const lineAst = (0, import_hastscript2.h)(`div.${codeLineClass}`);
    const gutterWrapper = renderedGutterElements.length ? (0, import_hastscript2.h)("div.gutter", renderedGutterElements) : void 0;
    if (gutterWrapper)
      lineAst.children.push(gutterWrapper);
    const codeWrapper = (0, import_hastscript2.h)("div.code");
...
function buildCodeBlockAstFromRenderedLines(codeBlock, renderedLines) {
  const preElement = (0, import_hastscript3.h)("pre", { tabindex: 0 }, (0, import_hastscript3.h)("code", renderedLines));
...
// src/internal/css.ts
var import_postcss2 = __toESM(require("postcss"), 1);
...
var preprocessor = (0, import_postcss2.default)([
...
var processor = (0, import_postcss2.default)([
...
async function scopeAndMinifyNestedCss(css) {
  const postCssOptions = { from: void 0 };
  const root = import_postcss2.default.parse(`.${groupWrapperClassName}{${css}}`, postCssOptions);
...
var convertLchToLab = ({ l, c, h: h6, alpha }, mode = "lab") => {
  let res = {
    mode,
    l,
    a: c ? c * Math.cos(h6 / 180 * Math.PI) : 0,
    b: c ? c * Math.sin(h6 / 180 * Math.PI) : 0
  };
  if (alpha !== void 0)
    res.alpha = alpha;
  return res;
...
function convertHslToRgb({ h: h6, s, l, alpha }) {
  h6 = normalizeHue_default(h6);
  let m1 = l + s * (l < 0.5 ? l : 1 - l);
  let m2 = m1 - (m1 - l) * 2 * Math.abs(h6 / 60 % 2 - 1);
  let res;
  switch (Math.floor(h6 / 60)) {
    case 0:
...
function lighten(input, amount) {
  return withParsedColor(input, (color) => {
    ...
    const { h: h6, s, a: alpha } = hsl;
    return toHexColor(toTinyColor({ mode: "hsl", h: h6, s, l: minMaxRounded(l + l * amount), alpha }));
  });
}
...
var import_hastscript5 = require("hastscript");
var corePlugins = [
  {
    name: "Indent Wrapper",
    hooks: {
        ...
        codeBlock.getLines().forEach((line) => {
          ...
          if (indent > 0) {
            line.getAnnotations().forEach((annotation) => {
              ...
              if (!inlineRange || !(annotation instanceof InlineStyleAnnotation))
                return;
...
var IndentAnnotation = class extends ExpressiveCodeAnnotation {
  render({ nodesToTransform }) {
    return nodesToTransform.map((node) => (0, import_hastscript5.h)("span.indent", node));
  }
};
