...
function renderLineToAst({
  ...
  let lineNode = h2(`div.${codeLineClass}`);
  if (renderedGutterElements.length) {
    lineNode.children.push(h2("div.gutter", renderedGutterElements));
  }
  lineNode.children.push(h2("div.code", partNodes.length > 0 ? partNodes : h2(null, "\n")));
  annotations.forEach((annotation) => {
    if (annotation.inlineRange)
      return;
...
function getRenderEmptyLineFn(context) {
  return () => {
    ...
    const lineAst = h2(`div.${codeLineClass}`);
    const gutterWrapper = renderedGutterElements.length ? h2("div.gutter", renderedGutterElements) : void 0;
    if (gutterWrapper)
      lineAst.children.push(gutterWrapper);
    const codeWrapper = h2("div.code");
...
function buildCodeBlockAstFromRenderedLines(codeBlock, renderedLines) {
  const preElement = h3("pre", { tabindex: 0 }, h3("code", renderedLines));
...
import { h as h5 } from "hastscript";
var corePlugins = [
  {
    name: "Indent Wrapper",
    hooks: {
...
var IndentAnnotation = class extends ExpressiveCodeAnnotation {
  render({ nodesToTransform }) {
    return nodesToTransform.map((node) => h5("span.indent", node));
  }
};
