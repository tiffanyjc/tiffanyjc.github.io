...
var TextMarkerAnnotation = class extends import_core2.ExpressiveCodeAnnotation {
  markerType;
  backgroundColor;
  label;
  ...
  renderFullLineMarker({ nodesToTransform }) {
    return nodesToTransform.map((node) => {
      if (node.type === "element") {
        (0, import_core2.addClassName)(node, "highlight");
        (0, import_core2.addClassName)(node, this.markerType);
        if (this.label) {
          (0, import_core2.addClassName)(node, "tm-label");
          (0, import_core2.setInlineStyle)(node, "--tmLabel", `'${this.label.replace(/'/g, "\\'")}'`);
        }
      }
      return node;
    });
  ...
  renderInlineMarker({ nodesToTransform }) {
    return nodesToTransform.map((node, idx) => {
      ...
      if (nodesToTransform.length > 0 && idx > 0) {
        (0, import_core2.addClassName)(transformedNode, "open-start");
      }
      if (nodesToTransform.length > 0 && idx < nodesToTransform.length - 1) {
        (0, import_core2.addClassName)(transformedNode, "open-end");
      }
      ...
      postprocessAnnotations: ({ codeBlock, styleVariants, config }) => {
        ...
        codeBlock.getLines().forEach((line) => {
          ...
          for (const annotation of annotations) {
            ...
            if (fullLineMarker) {
              if (MarkerTypeOrder.indexOf(annotation.markerType) < MarkerTypeOrder.indexOf(fullLineMarker.markerType))
                continue;
              if (import_core3.AnnotationRenderPhaseOrder.indexOf(annotation.renderPhase) < import_core3.AnnotationRenderPhaseOrder.indexOf(fullLineMarker.renderPhase))
                continue;
            }
          ...
          styleVariants.forEach((styleVariant, styleVariantIndex) => {
            ...
            const lineBgColor = (0, import_core3.onBackground)(fullLineMarkerBgColor, (0, import_core3.getStaticBackgroundColor)(styleVariant));
            const textColors = annotations.filter(
              (annotation) => annotation instanceof import_core3.InlineStyleAnnotation && annotation.color && // Only consider annotations that apply to the current style variant
            ...
            textColors.forEach((textColor) => {
              ...
              markers.forEach((marker) => {
                ...
                const combinedBgColor = (0, import_core3.onBackground)(inlineMarkerBgColor, lineBgColor);
                const readableTextColor = (0, import_core3.ensureColorContrastOnBackground)(textFgColor, combinedBgColor, config.minSyntaxHighlightingColorContrast);
                if (readableTextColor.toLowerCase() === textFgColor.toLowerCase())
                  return;
                line.addAnnotation(
                  new import_core3.InlineStyleAnnotation({
                    styleVariantIndex,
                    inlineRange: {
